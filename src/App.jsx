...
import { BUSINESS_DATA_FROM_SUPABASE, AGENDA_FROM_SUPABASE, MESSAGES_FROM_SUPABASE } from './dataSourceFlags';
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: "'Nunito Sans Variable', 'Nunito Sans', sans-serif", color: INK }}>
      ...
      <div className="max-w-md mx-auto" style={{ minHeight: '100vh', position: 'relative', background: BG }}>
        ...
        {/* Correction post-livraison (contre-vérification indépendante) : ce bandeau signale un
            état FONCTIONNEL réel (Partages/La Bande tournent encore sur des données locales
            identiques pour toutes les communautés), pas un artefact de développement — il ne
            doit donc jamais dépendre du mode build (DEMO_MODE/import.meta.env.DEV), sans quoi
            un build `npm run build` déployé tel quel afficherait silencieusement de fausses
            données sans aucun avertissement. Seule condition : l'état réel des données.
            ...
            ("Interdiction de livraison trompeuse", brief V7.7). Seuls Partages et La Bande
            restent concernés par ce bandeau dans ce lot. */}
        {!BUSINESS_DATA_FROM_SUPABASE && (
          <div style={{ margin: '10px 20px 0', background: '#EAF1FB', border: `1px solid ${BLUE}33`, borderRadius: 10, padding: '8px 12px', fontSize: 11.5, color: INK }}>
            Partages et La Bande affichent un contenu de démonstration — identique pour
            toutes les communautés tant qu'ils ne sont pas encore branchés à Supabase. L'Agenda
            et les Messages, eux, affichent déjà les vraies données de ta communauté.
          </div>
        )}
        ...
        {!loading && (
          <>
            ...
            {view === 'member-detail' && <MemberDetail memberId={selectedMemberId} onBack={() => setView(memberReturnTo)} />}
          </>
        )}

...
export default function App({ activeCommunity, memberships }) {
  ...
  // V7.37 (25 sept.) — coordonnées téléphone/email + partage : `members.phone_number`/`email`/
  // `share_whatsapp`/`share_phone`/`share_sms`/`share_email` existent désormais réellement en
  // base (sql/13_coordonnees_contact.sql) et sont lus par membersApi.js/memberDirectory.js
  // comme n'importe quel autre champ de l'annuaire réel — donc directement disponibles sur
  // chaque `member`/`me`, plus besoin d'un état séparé ici. Supprime l'ancien `meShareFlags`/
  // `toggleMeShareFlag` (local à la session, jamais persisté) : MyProfileSheet.jsx appelle
  // désormais membersApi.updateMyContact() directement, comme childrenApi.js/avatarApi.js.
